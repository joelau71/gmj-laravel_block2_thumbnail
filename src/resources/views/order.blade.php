<x-admin.layout.app>
    ...
    $breadcrumbs = [
        ...
        ['name' => $element->title],
        ['name' => "Thumbnail Content", "link" => route('LaravelBlock2Thumbnail.index', $element_id)],
        ['name' => "Order"]
    ];
    @endphp
    ...
    <form
        id="myForm"
        method="POST"
        action="{{ route('LaravelBlock2Thumbnail.order2', $element_id) }}"
    >
        @csrf
        <div id="menu-list">
            @foreach ($collections as $item)
                <div class="py-3 px-6 bg-black text-white rounded-md w-full mt-4 cursor-move">
                    <img src="{{ $item->getMedia('laravel_block2_thumbnail')->first()->getUrl() }}" alt="" class="w-20">
